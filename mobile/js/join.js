var strVar = "";
    ...
    strVar += "				<span style=\"color: #F85670\">加<\/span>";
    strVar += "				<span>入我们<\/span>";
    strVar += "			<\/div>";
    ...
    strVar += "                 <option value=\"\">请选择部门<\/option>";
    strVar += "					<option value=\"1\">产品研发部<\/option>";
    strVar += "                 <option value=\"2\">技术研发部<\/option>";
    strVar += "                 <option value=\"3\">硬件维修部<\/option>";
    strVar += "                 <option value=\"4\">创意策划部<\/option>";
    strVar += "				<\/select>";
    strVar += "				<div>";
    strVar += "					<input type=\"number\" name=\"student_id\" onclick=\"hideNotify();\" placeholder=\"请输入学号\" maxlength=\"16\"><\/div>";
    strVar += "				<div>";
    ...
    strVar += "				<div>";
    strVar += "					<input type=\"number\" name=\"phone_number\" onclick=\"hideNotify();\" placeholder=\"请输入手机号\"><\/div>";
    strVar += "				<div>";
...
$(function(){
    ...
    $("body").on("click","#join",function(){
        ...
        $.ajax({
            url:"http://localhost/cert/joinApi.php??act=join",
            type:"post",
            ...
            success:function(status){
                if (status.status == 1) {
                	...
                	$("#success").show();
                	$("#j-close").delay(3000).click();

                }else{
                	$("#success div").html(":(<br /> 失败<br />" + status.msg);
                	$("#success").show();
                	$("#j-close").delay(3000).hide();
                }
            }
        });
    });
    $.ajax({
        url:"http://localhost/cert/joinApi.php?act=getDepart",
        type:"get",
        dataType:"json",
